...
export default function Restaurants() {
  ...
  const form = useForm<z.infer<typeof reservationSchema>>({
    ...
    defaultValues: { date: "", time: "", guests: 2, guestName: "", guestEmail: "", specialRequests: "" },
  });
  ...
  const onSubmit = (restaurantId: number, data: z.infer<typeof reservationSchema>) => {
    reserveMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: t.restaurants.reservationConfirmed, description: t.restaurants.lookForward });
          form.reset();
        },
        onError: () => {
          toast({ variant: "destructive", title: t.restaurants.reservationFailed, description: t.restaurants.tryAgain });
        },
      }
    );
  };
  ...
  return (
    <div className="min-h-screen w-full pt-32 pb-24 bg-background">
      <div className="container mx-auto px-6 md:px-12 max-w-7xl">
        ...
        >
          <h1 className="text-4xl md:text-6xl font-serif text-foreground mb-6">{t.restaurants.heading}</h1>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="sm:max-w-[500px] bg-card border border-primary/30 rounded-none p-0 overflow-hidden">
                      <div className="bg-background border-b border-primary/20 p-6 text-center">
                        <DialogTitle className="text-2xl font-serif text-foreground font-normal">{t.restaurants.makeReservation} — {restaurant.name}</DialogTitle>
                      </div>
                      <div className="p-6">
                        <Form {...form}>
                          <form onSubmit={form.handleSubmit((data) => onSubmit(restaurant.id, data))} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                              <FormField control={form.control} name="date" render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-xs uppercase tracking-widest text-muted-foreground">{t.restaurants.date}</FormLabel>
                                  <FormControl>
                              ...
                              <FormField control={form.control} name="time" render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-xs uppercase tracking-widest text-muted-foreground">{t.restaurants.time}</FormLabel>
                                  <FormControl>
                            ...
                            <FormField control={form.control} name="guests" render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs uppercase tracking-widest text-muted-foreground">{t.restaurants.guests}</FormLabel>
                                <Select onValueChange={field.onChange} defaultValue={field.value.toString()}>
                                  <FormControl>
                                    <SelectTrigger className="rounded-none border-primary/30 bg-background h-12">
                                      <SelectValue />
                                    </SelectTrigger>
                                  </FormControl>
                            ...
                            <FormField control={form.control} name="guestName" render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs uppercase tracking-widest text-muted-foreground">{t.restaurants.name}</FormLabel>
                                <FormControl>
                            ...
                            <FormField control={form.control} name="guestEmail" render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs uppercase tracking-widest text-muted-foreground">{t.restaurants.email}</FormLabel>
                                <FormControl>
                            ...
                            >
                              {reserveMutation.isPending ? "..." : t.restaurants.confirm}
                            </Button>
                          </form>
                        </Form>
                      </div>
